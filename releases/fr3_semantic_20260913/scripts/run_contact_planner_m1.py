...
parser.add_argument(
    "--candidate-pose-score", choices=("legacy", "acceptance_scaled"), default="legacy",
    help="Opt-in acceptance-scaled pose reranking; candidate generation and execution are unchanged.",
)
...
parser.add_argument("--control-rate-hz", type=int, choices=(10, 20), default=10,
    help="FR3 action cadence; all duration step arguments retain their legacy 10 Hz meaning")
args_cli = parser.parse_args()
from m1_control_timing import retime
timing_contract = retime(args_cli, args_cli.control_rate_hz)
timing_contract.update({'physics_hz': 240, 'physics_dt_s': 0.004166666666666667, 'action_rate_hz': 20, 'action_dt_s': 0.05, 'decimation': 12, 'dwell_steps': 120, 'virtual_servo_hz': 1000, 'servo_ticks_per_action': [4, 4, 4, 4, 4, 5, 4, 4, 4, 4, 4, 5]})
(Path(os.environ["M1_COMPARISON_OUTPUT"]) / "timing_contract.json").write_text(
    json.dumps(timing_contract, indent=2) + "\n")
if args_cli.video:
...
from m1_candidate_pose_score import acceptance_scaled_pose_score
from fr3_execution_runtime.servo_candidate import check_candidate, rank_candidates
from fr3_execution_runtime.approach_route import route_admissible
import m1_fr3_comparison_adapter as comparison_adapter
import fr3_constraint_tasks
import isaaclab
import numpy as np
...
class FrankaEndpointIK:
    ...
    def __init__(self, max_evaluations: int) -> None:
        ...
        self.model = pin.buildModelFromUrdf(os.environ["DAPL_LOCAL_FRANKA_URDF"])
    ...
    def forward(self, q_arm: np.ndarray) -> pin.SE3:
        ...
        q_full = np.concatenate((q_arm, np.array((0.0, 0.0))))
...
def _pose_errors(base) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
    ...
    goal = getattr(base,"_semantic_final_goal",base.command_manager.get_command("target_object_pose"))
...
def main() -> None:
    ...
    comparison_adapter.configure(env_cfg, args_cli.control_rate_hz)
    env = gym.make(
        args_cli.task,
        cfg=env_cfg,
    ...
    try:
        ...
        comparison_finish = comparison_adapter.install(base, ik)
        ...
        from fr3_semantic_cost_map import snapshot, candidate_costs, micro_check, initialize_stages, update_stages, phase_check
        snapshot(base,video_markers)
        initialize_stages(base)
        ...
        def execute_phase(
            ...
            if bool(plan_enabled.any()) and not phase_check(base,ik,q_start[0].detach().cpu().numpy(),q_end[0].detach().cpu().numpy()):
                comparison_finish("semantic_robot_path_rejected")
            ...
            *,
            ...
            for micro_index in range(maximum_chunks):
                ...
                for env_id in torch.nonzero(
                    ...
                    diagnostics[env_id].setdefault("axis_checks", []).append({
                        "goal_delta": goal_delta_xy.tolist(), "contact": contact_xy.tolist(),
                        "com": target_com_live[env_id, :2].detach().cpu().tolist(),
                        "yaw_error": float(current_minus_goal_yaw[env_id].item()),
                        "yaw_rate": float(yaw_rate[env_id].item()),
                        "axis": axis.tolist(), "goal_axis_cosine": goal_axis_cosine,
                        "requested_moment": requested if math.isfinite(requested) else None,
                        "achieved_moment": achieved if math.isfinite(achieved) else None})
                    if (
                        goal_axis_cosine
                    ...
                    if (
                        ...
                        continue
                    live_points_np = target_points_live[env_id].detach().cpu().numpy()
                    rg2 = max(float(np.mean(np.sum((live_points_np[:,:2]-target_center[:2])**2,axis=1))),1e-6)
                    moment = float(np.cross(contact_xy-target_center[:2],axis))
                    predicted_micro_yaw = planner_cfg.rotation_efficiency*args_cli.persistent_micro_distance_m*moment/rg2
                    if not micro_check(base,ik,q_current_np,solved,bridge,live_points_np,target_center,axis,args_cli.persistent_micro_distance_m,predicted_micro_yaw):
                        chunk_valid[env_id] = False
                        diagnostics[env_id]["release_reason"] = "semantic_map_micro_rejected"
                        continue
        ...
        assert base.num_envs == 1 and args_cli.physics_rollout_candidates == 0, "Frozen single-scene route trial"
        for replan_index in range(args_cli.max_replans):
            approach_waypoints = {}
            from fr3_execution_runtime.clutter_route import from_live_scene
            clutter_guard = from_live_scene(ik, base)
            ...
            for env_id in torch.nonzero(active, as_tuple=False).flatten().tolist():
                ...
                nominal_servo_checks = {}
                adaptive_scores: dict[int, float] = {}
                map_costs = {}  # Evaluate after nominal servo directions are known.
                for candidate_rank in range(candidates.valid.shape[1]):
                    ...
                    nominal_servo_checks[candidate_rank] = check_candidate(
                        goal_delta=(goal[env_id, :2]-target_position[env_id, :2]).detach().cpu().numpy(),
                        contact=candidates.contact_point[env_id, candidate_rank, :2].detach().cpu().numpy(),
                        com=target_center_np[:2], yaw_error=-float(yaw_error[env_id].item()),
                        yaw_rate=float(base.scene["target"].data.root_ang_vel_w[env_id, 2].item()),
                        fallback_axis=direction_xy,
                        yaw_gain=args_cli.persistent_yaw_moment_gain_m_per_rad,
                        rate_gain=args_cli.persistent_yaw_rate_moment_gain_m_s_per_rad,
                        max_deviation=args_cli.persistent_max_axis_deviation_rad,
                        minimum_cosine=args_cli.persistent_minimum_goal_axis_cosine,
                        maximum_moment_error=args_cli.persistent_max_moment_arm_error_m)
                    direction_xy = np.asarray(nominal_servo_checks[candidate_rank]["axis"], dtype=float)
                    raw_yaw_response = distance * nominal_servo_checks[candidate_rank]["achieved_moment"] / planar_gyration_sq
                    ...
                    if args_cli.candidate_pose_score == "acceptance_scaled":
                        adaptive_scores[candidate_rank] = acceptance_scaled_pose_score(
                            float(np.linalg.norm(residual_xy)), predicted_yaw_error,
                            float(candidates.score[env_id, candidate_rank].item()),
                            yaw_is_inside=yaw_is_inside,
                            yaw_guard_rad=args_cli.predicted_yaw_guard_rad,
                        )
                map_costs = candidate_costs(base,candidates,target_points,target_com_position,planner_cfg.rotation_efficiency,planar_gyration_sq,servo_checks=nominal_servo_checks)
                adaptive_scores = {rank: score for rank,score in adaptive_scores.items() if bool(candidates.valid[env_id,rank])}
                task_scores_before_map = dict(adaptive_scores)
                for candidate_rank in adaptive_scores:
                    adaptive_scores[candidate_rank] += map_costs.get(candidate_rank,0.)
                from fr3_semantic_cost_map import record as semantic_record
                semantic_record(base,"ranking",task_scores_m=task_scores_before_map,semantic_costs_m=map_costs,total_scores_m=dict(adaptive_scores))
                analytic_rank_order = sorted(adaptive_scores, key=adaptive_scores.get)
                preferred_order = rank_candidates(adaptive_scores, nominal_servo_checks)
                plan_diagnostics[env_id].append({"event": "candidate_servo_shadow", "replan": replan_index,
                    "legacy_order": analytic_rank_order, "preferred_order": preferred_order,
                    "checks": nominal_servo_checks, "scores": adaptive_scores})
                if True:
                    analytic_rank_order = preferred_order
                if args_cli.physics_rollout_candidates > 0:
                ...
                for rank in rank_order:
                    ...
                    diagnostic["nominal_servo_check"] = nominal_servo_checks[rank]
                    if not ik_valid:
                    ...
                    if nominal_servo_checks[rank]["compatible"] and not preflight_valid:
                        direct_profile = []
                        for alpha in np.linspace(0., 1., args_cli.path_samples):
                            probe_q = (1-alpha)*q_current_np + alpha*solved[0]
                            probe_tcp, probe_rot = ik.tcp_pose_in_env(probe_q, bridge)
                            probe_hand = hand_local_np @ probe_rot.T + probe_tcp
                            direct_profile.append({"alpha": float(alpha),
                                "target_clearance_m": _minimum_point_distance(probe_hand, points_np),
                                "support_clearance_m": float(probe_hand[:,2].min()-points_np[:,2].min())})
                        lifted_tcp = endpoints[0].detach().cpu().numpy().copy()
                        lifted_tcp[2] += .05
                        probe_mid, probe_pos_err, probe_rot_err = ik.solve(
                            seed=q_current_np, regularization_reference=q_current_np,
                            bridge=bridge, desired_tcp_env=lifted_tcp, desired_rotation_env=rotation_np)
                        probe_segments = []
                        for probe_a, probe_b in ((q_current_np, probe_mid), (probe_mid, solved[0])):
                            probe_segments.append({
                                "target_clearance_m": _joint_segment_semantic_clearance(ik,
                                    q_start=probe_a, q_end=probe_b, bridge=bridge,
                                    hand_points_local=hand_local_np, target_points=points_np,
                                    target_translation=np.zeros(3), target_center=target_center_np,
                                    target_yaw_delta=0., samples=max(32, args_cli.path_samples)),
                                "support_clearance_m": _joint_segment_support_clearance(ik,
                                    q_start=probe_a, q_end=probe_b, bridge=bridge,
                                    hand_points_local=hand_local_np, support_height=float(points_np[:,2].min()),
                                    samples=max(32, args_cli.path_samples))})
                        diagnostic["approach_probe"] = {"direct_profile": direct_profile,
                            "lift_m": .05, "waypoint_q": probe_mid.tolist(),
                            "ik_position_error_m": probe_pos_err, "ik_rotation_error_rad": probe_rot_err,
                            "segments": probe_segments}
                    if "approach_probe" in diagnostic:
                        probe = diagnostic["approach_probe"]
                        repaired = route_admissible(preflight_values, probe["segments"],
                            position_error=probe["ik_position_error_m"], rotation_error=probe["ik_rotation_error_rad"],
                            position_tolerance=args_cli.ik_position_tolerance_m,
                            rotation_tolerance=args_cli.ik_rotation_tolerance_rad,
                            approach_clearance=planner_cfg.approach_clearance_m,
                            forbidden_clearance=planner_cfg.forbidden_clearance_m,
                            support_clearance=planner_cfg.support_clearance_m,
                            contact_distance=planner_cfg.contact_distance_m)
                        diagnostic["direct_preflight_valid"] = preflight_valid
                        diagnostic["approach_route_admitted"] = repaired
                        if repaired:
                            preflight_valid = True
                            approach_waypoints[(env_id, rank)] = probe["waypoint_q"]
                    if clutter_guard is not None and preflight_valid:
                        route_mid = approach_waypoints.get((env_id, rank))
                        route_nodes = [q_current_np] + ([] if route_mid is None else [np.asarray(route_mid)]) + [solved[0]]
                        direct_checks = [clutter_guard.segment(a, b, bridge) for a,b in zip(route_nodes,route_nodes[1:])]
                        tail_checks = [clutter_guard.segment(a, b, bridge) for a,b in zip(solved,solved[1:])]
                        diagnostic['clutter_preflight'] = {'approach': direct_checks, 'contact_push_retreat': tail_checks, 'margin_m': .012, 'robot_geometry_count': len(clutter_guard.names), 'static_obstacle_boxes': True}
                        tail_valid = all(x['clearance_m'] > .012 for x in tail_checks)
                        preflight_valid = tail_valid and all(x['clearance_m'] > .012 for x in direct_checks)
                        if not preflight_valid and tail_valid:
                            probes = []
                            # Finite family; retain original target/support and IK requirements.
                            for lift, side in ((.10,0.),(.20,0.),(.30,0.),(.15,-.12),(.15,.12),(.25,-.12),(.25,.12)):
                                waypoint = (tcp_position[env_id].detach().cpu().numpy()+endpoints[0].detach().cpu().numpy())*.5
                                waypoint[2] = max(float(tcp_position[env_id,2]), float(endpoints[0][2]))+lift
                                waypoint[0] += side
                                mid, pe, re = ik.solve(seed=q_current_np, regularization_reference=q_current_np, bridge=bridge, desired_tcp_env=waypoint, desired_rotation_env=rotation_np)
                                probe = {'lift_m':lift,'side_m':side,'q':mid.tolist(),'position_error_m':pe,'rotation_error_rad':re}
                                probes.append(probe)
                                if pe > args_cli.ik_position_tolerance_m or re > args_cli.ik_rotation_tolerance_rad:
                                    continue
                                segments = []
                                for a,b in ((q_current_np,mid),(mid,solved[0])):
                                    segments.append(dict(clutter_guard.segment(a,b,bridge),
                                        target_clearance_m=_joint_segment_semantic_clearance(ik,q_start=a,q_end=b,bridge=bridge,hand_points_local=hand_local_np,target_points=points_np,target_translation=np.zeros(3),target_center=target_center_np,target_yaw_delta=0.,samples=64),
                                        support_clearance_m=_joint_segment_support_clearance(ik,q_start=a,q_end=b,bridge=bridge,hand_points_local=hand_local_np,support_height=float(points_np[:,2].min()),samples=64)))
                                probe['segments'] = segments
                                admitted = all(x['clearance_m'] > .012 for x in segments) and route_admissible(preflight_values,[{k:x[k] for k in ("target_clearance_m","support_clearance_m")} for x in segments],position_error=pe,rotation_error=re,position_tolerance=args_cli.ik_position_tolerance_m,rotation_tolerance=args_cli.ik_rotation_tolerance_rad,approach_clearance=planner_cfg.approach_clearance_m,forbidden_clearance=planner_cfg.forbidden_clearance_m,support_clearance=planner_cfg.support_clearance_m,contact_distance=planner_cfg.contact_distance_m)
                                probe['admitted'] = admitted
                                if admitted:
                                    preflight_valid = True
                                    approach_waypoints[(env_id,rank)] = mid.tolist()
                                    break
                            diagnostic['clutter_preflight']['detours'] = probes
                        diagnostic['clutter_preflight']['admitted'] = preflight_valid
                        diagnostic['clutter_preflight']['distance_queries_total'] = clutter_guard.queries
            ...
            route_q = approach_waypoints.get((0, int(selected_rank[0].item())))
            if route_q is None:
                (
                    approach_contact,
                    q_approach_contact,
                ...
                q_mid = torch.as_tensor([route_q], dtype=current_q.dtype, device=base.device)
                first_steps = args_cli.approach_steps // 2
                assert first_steps > 0
                first_hit, first_q, first_safe, first_forbidden = execute_until_safe_contact(
                    current_q, q_mid, first_steps, plan_enabled, 0)
                second_hit, second_q, second_safe, second_forbidden = execute_until_safe_contact(
                    q_mid, q_pre, args_cli.approach_steps-first_steps,
                    plan_enabled & ~first_hit & ~constraint_violation_ever, 0)
                approach_contact = first_hit | second_hit
                q_approach_contact = torch.where(first_hit[:, None], first_q, second_q)
                approach_safe_distance = torch.where(first_hit, first_safe, second_safe)
                approach_forbidden_distance = torch.where(first_hit, first_forbidden, second_forbidden)
                plan_diagnostics[0].append({"event": "raised_approach_executed", "replan": replan_index,
                    "candidate_rank": int(selected_rank[0].item()), "waypoint_q": route_q,
                    "first_steps": first_steps, "second_steps": args_cli.approach_steps-first_steps,
                    "first_leg_contact": bool(first_hit[0].item())})
            ...
            if clutter_guard is not None and bool(escape_enabled[0]):
                # The executor uses a measured vertical lift, not the nominal
                # candidate's q_retreat. Validate the actual return path too.
                return_guard = from_live_scene(ik, base)
                measured_q = robot.data.joint_pos[:, joint_ids].clone()
                start_np = measured_q[0].detach().cpu().numpy()
                home_np = current_q[0].detach().cpu().numpy()
                return_points = get_object_pointcloud_in_env_frame(base, SceneEntityCfg("target")).reshape(base.num_envs,-1,3)[0].detach().cpu().numpy()
                return_hand = hand_local_np
                return_bridge = bridges[0]
                def check_return(nodes):
                    checks = []
                    for a,b in zip(nodes,nodes[1:]):
                        checks.append(dict(return_guard.segment(a,b,return_bridge),
                            target_clearance_m=_joint_segment_semantic_clearance(ik,q_start=a,q_end=b,bridge=return_bridge,hand_points_local=return_hand,target_points=return_points,target_translation=np.zeros(3),target_center=return_points.mean(axis=0),target_yaw_delta=0.,samples=64),
                            support_clearance_m=_joint_segment_support_clearance(ik,q_start=a,q_end=b,bridge=return_bridge,hand_points_local=return_hand,support_height=float(return_points[:,2].min()),samples=64)))
                    return checks
                nodes = [start_np,home_np]
                checks = check_return(nodes)
                direct_return_checks = checks
                def admissible_return(checks):
                    return all(x['clearance_m']>.012 and x['target_clearance_m']>planner_cfg.approach_clearance_m and x['support_clearance_m']>planner_cfg.support_clearance_m for x in checks)
                if not admissible_return(checks) and route_q is not None:
                    nodes = [start_np,np.asarray(route_q),home_np]
                    checks = check_return(nodes)
                admitted = admissible_return(checks)
                plan_diagnostics[0].append({'event':'clutter_return_preflight','replan':replan_index,'direct':direct_return_checks,'selected':checks,'admitted':admitted,'physics_step':int(base._sim_step_counter) if hasattr(base,'_sim_step_counter') else None})
                if admitted:
                    budget = args_cli.approach_steps
                    for leg,(a,b) in enumerate(zip(nodes,nodes[1:])):
                        steps = budget//(len(nodes)-1) if leg<len(nodes)-2 else budget-budget//(len(nodes)-1)*leg
                        execute_phase(torch.as_tensor(a[None],device=base.device,dtype=current_q.dtype),torch.as_tensor(b[None],device=base.device,dtype=current_q.dtype),steps,escape_enabled)
                else:
                    exhausted |= escape_enabled
            else:
                execute_phase(
                    escape_start,
                    current_q,
        ...
        comparison_finish("planner_finished")
    finally:
        if "plan_diagnostics" in locals():
            (Path(os.environ["M1_COMPARISON_OUTPUT"]) / "partial_plans.json").write_text(
                json.dumps(_strict_json_value({"plans": plan_diagnostics}), indent=2, allow_nan=False) + "\n")
        if selective_video_writer is not None:
