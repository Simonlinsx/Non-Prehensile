...
def resolve_domino_asset(asset_id: str, root: str | Path | None = None) -> tuple[Path, Path]:
    ...
    if asset_id.split(':')[0] in ('901_doll','902_balloon','903_sponge','904_liquidcup','905_bowl'):
        folder = Path(os.environ['FR3_SEMANTIC_ASSETS']) / asset_id.split(':')[0]
        return folder/'asset.usda', folder/'geometry.obj'
