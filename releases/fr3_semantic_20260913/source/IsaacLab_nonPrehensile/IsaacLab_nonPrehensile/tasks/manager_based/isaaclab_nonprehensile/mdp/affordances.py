...
def _robot_target_affordance_geometry(
    ...
    *,
    ...
    distance_function = _chunked_target_semantic_distances
    if (getattr(env, "_c3_semantic_distance_cuda_graph_enabled", False)
            and end_effector_points.is_contiguous() and target_points.is_contiguous()):
        from .semantic_distance_graph import SemanticDistanceGraph
        runner = getattr(env, "_c3_semantic_distance_cuda_graph", None)
        if runner is None:
            runner = SemanticDistanceGraph(
                distance_function, end_effector_points, target_points,
                (safe_mask, forbidden_mask, neutral_mask, protected_mask),
            )
            env._c3_semantic_distance_cuda_graph = runner
        distance_function = runner
    (
